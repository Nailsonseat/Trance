<template>
    ...
    <div class="position-absolute top-50 start-50 translate-middle modal-content">
        ...
        <div class="modal-header">
            <h5 class="modal-title fs-4">Assign Songs to album</h5>
        ...
        <div class="modal-body">
            ...
            <label class="my-4 fs-4" for="coverPic">Update Album</label>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-primary" @click="updateAlbum">Update
                Album</button>
...
export default {
    ...
    beforeMount() {
        this.fetchAlbumSongs(this.albumId);
    },
    methods: {
        updateAlbum() {
            ...
            axios.put(API_ENDPOINT + `/albums/${this.albumId}/assign`, { song_ids: songIds })
                .then(response => {
                    console.log('Album songs updated successfully:', response.data);
                    this.closeModal();
                })
                .catch(error => {
        ...
        fetchAlbumSongs(albumId) {
            const API_ENDPOINT = 'http://localhost:5000/';
            axios.get(API_ENDPOINT + `/albums/${this.albumId}/assign`)
                .then(response => {
                    const songIds = response.data.song_ids;
                    this.selectedSongs = this.songs.filter(song => songIds.includes(song.id));
                    // Do something with the song IDs, such as updating your component's data
                })
                .catch(error => {
